...
typedef struct {
    ...
    XCreat_mec_matrix_uio_map maps[ MAX_UIO_MAPS ];
} XCreat_mec_matrix_uio_info;

/***************** Variable Definitions **************************************/
static XCreat_mec_matrix_uio_info uio_info;
...
static int uio_info_read_name(XCreat_mec_matrix_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XCreat_mec_matrix_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XCreat_mec_matrix_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XCreat_mec_matrix_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XCreat_mec_matrix_Initialize(XCreat_mec_matrix *InstancePtr, const char* InstanceName) {
	XCreat_mec_matrix_uio_info *InfoPtr = &uio_info;
