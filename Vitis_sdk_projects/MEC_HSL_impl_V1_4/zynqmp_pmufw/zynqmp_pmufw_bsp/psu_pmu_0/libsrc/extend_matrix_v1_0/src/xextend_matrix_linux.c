...
typedef struct {
    ...
    XExtend_matrix_uio_map maps[ MAX_UIO_MAPS ];
} XExtend_matrix_uio_info;

/***************** Variable Definitions **************************************/
static XExtend_matrix_uio_info uio_info;
...
static int uio_info_read_name(XExtend_matrix_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XExtend_matrix_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XExtend_matrix_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XExtend_matrix_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XExtend_matrix_Initialize(XExtend_matrix *InstancePtr, const char* InstanceName) {
	XExtend_matrix_uio_info *InfoPtr = &uio_info;
