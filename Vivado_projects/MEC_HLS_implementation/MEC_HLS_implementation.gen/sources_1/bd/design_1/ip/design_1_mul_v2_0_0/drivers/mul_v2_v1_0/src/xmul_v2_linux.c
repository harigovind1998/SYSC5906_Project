...
typedef struct {
    ...
    XMul_v2_uio_map maps[ MAX_UIO_MAPS ];
} XMul_v2_uio_info;

/***************** Variable Definitions **************************************/
static XMul_v2_uio_info uio_info;
...
static int uio_info_read_name(XMul_v2_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XMul_v2_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XMul_v2_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XMul_v2_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XMul_v2_Initialize(XMul_v2 *InstancePtr, const char* InstanceName) {
	XMul_v2_uio_info *InfoPtr = &uio_info;
